...
export async function POST(req: NextRequest) {
  return handleApi(req, async () => {
    ...
    const updateTx = conn.transaction(async (tx) => {
      await tx.prepare("UPDATE password_reset_tokens SET used_at = ? WHERE id = ?").run(ts, row.id);
      await tx
      ...
      await auditLog(tx, { action: "PASSWORD_RESET", actorUserId: row.user_id, targetUserId: row.user_id, ip: getClientIp(req) });
    });
    await updateTx();
